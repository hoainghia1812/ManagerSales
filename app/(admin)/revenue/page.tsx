    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard
          ...
          change={formatCurrency(target - totalRevenue) + " còn lại"}
          changeType="increase"
      ...
      <Card>
        <CardHeader
          title="Mục tiêu doanh thu năm"
          subtitle={`${formatCurrency(totalRevenue)} / ${formatCurrency(target)}`}
        />
        <ProgressBar
          value={totalRevenue}
          max={target}
          showLabel
          color="bg-gradient-to-r from-brand-700 to-brand-400"
        />
                ...
                <Bar dataKey="revenue" radius={[8, 8, 0, 0]}>
                  {monthlyRevenue.map((_, index) => (
                    <Cell
                      ...
                      opacity={
                        0.4 +
                        (index / (monthlyRevenue.length - 1)) * 0.6
                      }
                    />
                  ))}
                </Bar>
