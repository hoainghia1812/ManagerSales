...
export default function ShopPage() {
  ...
  const activeProducts = productsWithDetails.filter((p) => p.is_active);
    ...
    <div>
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-6 py-16 md:py-24 lg:py-32 relative">
          <div className="max-w-2xl">
            ...
            <div className="flex flex-wrap gap-3 mt-6 md:mt-8">
              <Button size="lg" icon={<ShoppingBag size={18} />} className="shadow-lg shadow-brand-700/20">
                Khám phá ngay
              </Button>
              <Button variant="outline" size="lg" icon={<Gift size={18} />}>Quà tặng</Button>
            </div>
          </div>
        </div>
      </section>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {[
              ...
              <div key={i} className="flex items-center gap-3 justify-center md:justify-start">
                ...
                <span className="text-sm font-medium text-brand-600">{item.text}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="max-w-7xl mx-auto px-4 md:px-6 py-12 md:py-16">
        <div className="text-center mb-10">
          <h2 className="font-serif text-3xl md:text-4xl font-bold text-brand-700">Bộ sưu tập</h2>
          <p className="text-brand-500 mt-2 text-lg">Những sản phẩm thủ công được yêu thích nhất</p>
        </div>
      ...
      <section className="bg-brand-700 text-brand-100">
        <div className="max-w-7xl mx-auto px-4 md:px-6 py-16 md:py-20 text-center">
          ...
          <h2 className="font-serif text-3xl md:text-4xl font-bold mb-3">Quà tặng ý nghĩa</h2>
          <p className="text-brand-300 text-lg max-w-lg mx-auto mb-8">
            Hãy để MUSH&CO. giúp bạn trao gửi yêu thương qua những món quà thủ công tinh tế
          </p>
          <Button variant="secondary" size="lg" icon={<ArrowRight size={18} />} className="shadow-lg">
            Đặt quà ngay
          </Button>
        </div>
      </section>
    ...
    <div className="group bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-lg transition-all duration-300 border border-brand-100/50">
      <div className="aspect-square bg-linear-to-br from-brand-100 to-brand-200/30 relative overflow-hidden">
        <div className="absolute inset-0 flex items-center justify-center group-hover:scale-110 transition-transform duration-500">
          <Package size={64} className="text-brand-300/60" strokeWidth={0.8} />
        </div>
        ...
        <button
          ...
          className={`absolute top-3 right-3 w-9 h-9 rounded-full flex items-center justify-center transition-all duration-200 ${
            liked ? "bg-red-50 text-red-500" : "bg-white/80 text-brand-400 hover:bg-white hover:text-red-400"
          }`}
        >
      ...
      <div className="p-4 md:p-5">
        <div className="flex items-center gap-1 mb-1.5">
          {[...Array(5)].map((_, i) => (
            <Star key={i} size={12} className={i < 4 ? "text-brand-400" : "text-brand-200"} fill={i < 4 ? "currentColor" : "none"} strokeWidth={1.5} />
          ))}
          <span className="text-xs text-brand-400 ml-1">(4.0)</span>
        </div>
        <p className="text-xs text-brand-400 uppercase tracking-wider font-medium">{product.category?.name}</p>
        <h3 className="font-serif text-lg font-semibold text-brand-700 mt-1 leading-snug">{product.name}</h3>
        <p className="text-sm text-brand-500 mt-1 line-clamp-2">{product.description}</p>
        {product.variants && product.variants.length > 1 && (
          <p className="text-xs text-brand-400 mt-2">{product.variants.length} biến thể</p>
        )}
        <div className="flex items-center justify-between mt-4 pt-4 border-t border-brand-50">
          <div>
            {minPrice === maxPrice ? (
              <span className="text-xl font-bold text-brand-400 font-serif">{formatCurrency(minPrice)}</span>
            ) : (
              <span className="text-lg font-bold text-brand-400 font-serif">
                {formatCurrency(minPrice)} - {formatCurrency(maxPrice)}
              </span>
            )}
          </div>
          <Button size="sm" icon={<ShoppingBag size={14} />} className="shadow-sm" disabled={totalStock === 0}>
            Mua ngay
          </Button>
        </div>
      </div>
